import { Box, Text, useSegmentGroup } from "@chakra-ui/react";
import { getUserPathByUid } from "../services/users.service";
import { AppContext } from "../state/app.context";
import { useContext } from "react";
import { db } from "../config/firebase-config";
import { set, update, ref, get, push } from "firebase/database";
    ...
    <>
      {(intake.length > 0 || total) && (
        <Box mt={4} p={4} bg="gray.900" borderRadius="md">
          ...
          <Text>Calories: {(intake.length > 0 ? totalCalories : total?.calories)?.toFixed(1)}</Text>
          <Text>Protein: {(intake.length > 0 ? totalProtein : total?.protein)?.toFixed(1)}g</Text>
          <Text>Carbs: {(intake.length > 0 ? totalCarbs : total?.carbs)?.toFixed(1)}g</Text>
          <Text>Fat: {(intake.length > 0 ? totalFat : total?.fat)?.toFixed(1)}g</Text>
        </Box>
      )}
    </>
  );
